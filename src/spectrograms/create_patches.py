...
def prepare_testset(dataset_name):
    ...
    save_sparse_csr(common.DATASETS_DIR+'/matrix_test_%s.npz' % dataset_name,good_test_matrix)
    fw = open(common.DATASETS_DIR+'/items_index_test_%s.tsv' % dataset_name, "w")
    fw.write("\n".join(testset))
